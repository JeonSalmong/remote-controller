...
class RemoteClient:
    ...
    def _show_connect_dialog(self):
        ...
        def do_connect():
            ...
            port = int(port_var.get().strip() or '9999')
            pin = pin_var.get().strip()
            dialog.destroy()
            self.root.after(100, lambda: self._connect(host_ip, port, pin))

        tk.Button(dialog, text='연결', command=do_connect, width=12).grid(
            row=3, column=0, columnspan=2, pady=12)

        dialog.protocol('WM_DELETE_WINDOW', lambda: self.root.destroy())

    def _connect(self, host_ip: str, port: int, pin: str):
        if not host_ip or not pin:
            messagebox.showerror('오류', 'IP와 PIN을 입력해주세요')
            self._show_connect_dialog()
            return
        ...
        try:
            ...
            if resp.get('status') != 'ok':
                messagebox.showerror('인증 실패', 'PIN이 틀렸습니다!')
                self.sock.close()
                self._show_connect_dialog()
                return
            ...
            self.running = True
            self.status_var.set(f'연결됨: {host_ip}:{port}')
            self.root.title(f'원격 데스크톱 - {host_ip}')

            threading.Thread(target=self._recv_screen, daemon=True).start()

        except ConnectionRefusedError:
            messagebox.showerror('연결 실패', f'{host_ip}:{port} 에 연결할 수 없습니다.\n호스트가 실행 중인지 확인하세요.')
            self._show_connect_dialog()
        except OSError as e:
